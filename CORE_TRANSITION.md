...
- FFT placeholder: `gpu_vkfft_stub.py` provides `fft2/ifft2/has_vkfft` that fall back to NumPy with a warning when vkFFT bindings are absent; swap to real vkFFT when bindings are available on the target GPU. `gpu_vkfft_adapter.VkFFTExecutor` now exposes explicit backends: `numpy`, `vkfft-opencl` (pyvkfft), `vkfft-vulkan` (pybind11 bridge to DTolm/VkFFT), and `vkfft` (auto: prefer Vulkan, then OpenCL). CLI mirrors this via `scripts/run_fft_smoke.py --fft-backend`.
- Vulkan FFT binding status (RX 580 / RADV, 2026-01-24): pybind11 bridge builds against vendored VkFFT headers; smoke test now runs end-to-end with `VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/radeon_icd.x86_64.json` and selects `vkFFT/Vulkan`, but the reconstruction error is currently large (`~2.68e5` on 256×256 complex64). Treat this as a correctness bug in the Vulkan FFT path; NumPy fallback remains correct. Leave Vulkan FFT disabled for production until error is resolved.
...
- FFT path status (2026-01-24): `gpu_vkfft_adapter.py` now runs end-to-end with the Vulkan VkFFT binding. Clone `https://github.com/DTolm/VkFFT` (e.g., into `VkFFT/`), build the pybind11 wheel via `VKFFT_INCLUDE_DIR=VkFFT \` followed by `python setup_vkfft_vulkan.py bdist_wheel` and `pip install dist/vkfft_vulkan_py-*.whl`, then run `scripts/run_fft_smoke.py --fft-backend vkfft-vulkan` with `VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/radeon_icd.x86_64.json`. Current reconstruction error on RADV/RX 580 for 256×256 complex64 is ~1.6e-6 (acceptable parity). The adapter still logs once and falls back to NumPy if the binding fails or is absent.
- Current GPU backend status: `accelerated` is CPU-only; Vulkan hooks exist but are not wired into dashiCFD v4 pipeline in this repo. To actually use GPU masks/FFTs, they need our functional GPU kernels (completed now?) and to plumb `dashi_core.backend` into encode_proxy/mask ops. Once a real GPU backend is available, add a `--backend {cpu,vk}` flag in the runner and wrap masking in `with use_backend("vulkan_core_mask"):` to route through Vulkan.
