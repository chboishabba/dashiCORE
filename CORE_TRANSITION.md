...
- FFT path status: `gpu_vkfft_adapter.py` provides an optional vkFFT executor with NumPy fallback; bindings are not vendored. To enable Vulkan FFT, clone `https://github.com/DTolm/VkFFT` (e.g., into `third_party/VkFFT`) and build the pybind11 extension via `setup_vkfft_vulkan.py`, which consumes existing Vulkan handles. When bindings are missing or a probe crashes, the adapter logs once and falls back to NumPy. Default flows remain CPU-only unless explicitly wired.
